...
mobwrite.shareIframeObj = function(doc, id) {
  ...
  // Don't highlight incomming text on the first update.
  this.firstEdit = true;
};
 ...
 * @type {string}
 */
mobwrite.shareIframeObj.prototype.zapIcon = 'http://mobwrite3.appspot.com/static/zap.gif';
if (!mobwrite.UA_msie) {
...
/**
 * Array of current zap icon locations.  Used by IE because images count as one character.
 * @type {Array}
 */
mobwrite.shareIframeObj.zapIconList = [];
...
mobwrite.shareIframeObj.prototype.setClientText = function(value) {
  ...
  value = mobwrite.shareIframeObj.spaceToNbsp(value);
  if (mobwrite.UA_webkit) {
...
mobwrite.shareIframeObj.prototype.patchClientText = function(patches) {
  ...
  for (var x = 0; x < patches.length; x++) {
    ...
    // Run a diff to get a framework of equivalent indicies.
    diff = this.dmp.diff_main(text1, text2, false);
    index1 = 0;
    for (var y = 0; y < patches[x].diffs.length; y++) {
      ...
      if (mod[0] == DIFF_INSERT) {  // Insertion
        ...
        if (this.doc.createRange) {  // W3
          var startDom = mobwrite.shareIframeObj.findNodeOffset_(body, startLoc + index2);
          ...
          try {
            ...
            alert('Error on setStart during insert:\n' + startDom + '\n' + e);
            continue;
          }
          range.collapse(true);
          // Location is known, now add the text.
          span = this.doc.createElement('span');
          span.id = Math.random().toString().substring(2);
          span.className = 'highlight';
          mod[1] = mobwrite.shareIframeObj.spaceToNbsp(mod[1]);
          ...
          setTimeout('mobwrite.shareIframeObj.cropNode("' + this.file + '", "' + span.id + '")', 2000);
          range.detach();
          ...
          startDom = mobwrite.shareIframeObj.textLen(text.substring(0, startLoc + index2));
          ...
          span = mobwrite.shareIframeObj.spaceToNbsp(span);
          span = span.replace(/\r\n/g, '<BR>');
          spanId = Math.random().toString().substring(2);
          span = '<SPAN CLASS="highlight" ID="' + spanId + '">' + span + '</SPAN>'
          range.pasteHTML(span);
          setTimeout('mobwrite.shareIframeObj.cropNode("' + this.file + '", "' + spanId + '")', 2000);
        } else {
        ...
        if (this.doc.createRange) {  // W3
          ...
          try {
            ...
            alert('Error on setStart during delete:\n' + startDom + '\n' + e);
            continue;
          }
          try {
            if (endDom[1] == 0) {
            ...
            alert('Error on setEnd during delete:\n' + endDom + '\n' + e);
            continue;
          }
          range.deleteContents();
          // Add a zap icon if this delete is not followed by an insert.
          if (mobwrite.shareIframeObj.zapIcon && patches[x].diffs[y + 1] && patches[x].diffs[y + 1][0] != DIFF_INSERT) {
            span = this.doc.createElement('img');
            span.src = mobwrite.shareIframeObj.zapIcon;
            span.id = Math.random().toString().substring(2);
            span.className = 'highlight';
            range.insertNode(span);
            setTimeout('mobwrite.shareIframeObj.nukeZap("' + this.file + '", "' + span.id + '")', 1000);
          }
          range.detach();
        } else if (body.createTextRange) {  // MSIE
          if (this.decolourChanges) {
          ...
          startDom = mobwrite.shareIframeObj.textLen(text.substring(0, startLoc + index2));
          range.move('character', startDom);
          endDom = mobwrite.shareIframeObj.strLen(mod[1]);
          range.moveEnd('character', endDom);
          // Add a zap icon if this delete is not followed by an insert.
          if (mobwrite.shareIframeObj.zapIcon && patches[x].diffs[y + 1] && patches[x].diffs[y + 1][0] != DIFF_INSERT) {
            spanId = Math.random().toString().substring(2);
            span = '<IMG SRC="' + mobwrite.shareIframeObj.zapIcon + '" CLASS="highlight" ID="' + spanId + '">';
            range.pasteHTML(span);
            // Increment all larger zap locations by one.
            for (var z = 0; z < mobwrite.shareIframeObj.zapIconList.length; z++) {
              if (mobwrite.shareIframeObj.zapiconlist[z][1] > startDom) {
                mobwrite.shareIframeObj.zapiconlist[z][1]--;
              }
            }
            mobwrite.shareIframeObj.zapIconList.push([spanId, startDom]);
            setTimeout('mobwrite.shareIframeObj.nukeZap("' + this.file + '", "' + spanid + '")', 1000);
          } else {
...
// Ideally the node's children would be moved, but this an cause cursor jumps.
mobwrite.shareIframeObj.cropNode = function(shareId, nodeId) {
  var node = mobwrite.shared[shareId].doc.getElementById(nodeId);
  if (node) {
    node.className = '';
  }
};
...
mobwrite.shareIframeObj.nukeZap = function(shareId, nodeId) {
  // Find the zap in the current list of zap locations and delete it.
  // The mobwrite.shareIframeObj.zapiconlist will always be empty except for IE.
  var loc = null;
  var list = mobwrite.shareIframeObj.zapIconList;
  ...
  // Decrement all larger zap locations by one.
  if (loc != null) {
...
// Return the length of the text in IE's DOM.
mobwrite.shareIframeObj.textLen = function(text) {
  var len = mobwrite.shareIframeObj.strLen(text);
  // Each zap icon is counted as one extra character.
  var zaps = 0;
  for (var z = 0; z < mobwrite.shareIframeObj.zapIconList.length; z++) {
    if (mobwrite.shareIframeObj.zapIconList[z][1] <= len) {
      zaps++;
    }
  }
  return len + zaps;
...
// Return the length of a string in IE's DOM.
mobwrite.shareIframeObj.strLen = function(text) {
  // \r\n are counted as only one character.
  return text.replace(/\r/g, '').length;
};


// Add &nbsp; so that whitespace doesn't collapse.
mobwrite.shareIframeObj.spaceToNbsp = function(text) {
      ...
      if (node.nodeType == 3) {
        ...
        if (node.data) {
          if (text && crlf) {
            ...
            crlf[mobwrite.shareIframeObj.charAddAttribute] = 2;
            text += '\r\n';
          }
          crlf = null;
          br = false;
        ...
        if (node.tagName == 'DIV' || node.tagName == 'P') {
          if (mobwrite.UA_opera && node.tagName == 'P' &&
            ...
            text += '\r\n';
            node[mobwrite.shareIframeObj.charAddAttribute] = 2;
          ...
          if (text && crlf) {
            ...
            text += '\r\n';
            crlf[mobwrite.shareIframeObj.charAddAttribute] = 2;
            node[mobwrite.shareIframeObj.charIndexAttribute] += 2;
          }
          crlf = null;
          br = true;
          text += '\r\n';
...
mobwrite.shareIframeObj.followMapDom_ = function(node, target) {
  ...
  // Special case for BR: it creates two characters and does not contain nodes.
  // 'a<BR>b': 0->a, 1->a, 2-><BR>, 3->b, 4->b
  // 'a<P>b</P>': 0->a, 1->a, 2-><P>, 3-><P>, 4->b
  if (node.tagName == 'BR' && mobwrite.shareIframeObj.charAddAttribute in node &&
    ...
    return mobwrite.shareIframeObj.walkDown_(node);
  }
  ...
  // linefeed if that node is a P or DIV.
 ...
 * @return {Object?} A sharing object or null
 */
mobwrite.shareIframeObj.shareHandler = function(el) {
  ...
  var id = el.name;
  if (el.contentDocument) {  // Mozilla
    el = el.contentDocument;
  } else if (el.document) {  // IE
    el = el.document;
  } else {
  ...
  // Switch on editability.
  el.designMode = 'On';

  return new mobwrite.shareIframeObj(el, id);
};
